...
import { MagangApi } from "../../midleware/HrdApi";
...
const Magang = () => {
  ...
  const getDataList = async () => {
    try {
      const res = await MagangApi.showAll(token, {
        ...filter,
      });
  ...
  const getDivisions = async () => {
    try {
      const res = await MagangApi.recapByDivision(token);
  ...
  return (
    <>
      <Header links={[{ l: "HRD" }]} title="Magang" />
        ...
        {divisions
          ...
          .map((dat, i) => (
            <Stat
              ...
              className={
                "cursor-pointer " +
                (filter.division_id == dat.division_id
